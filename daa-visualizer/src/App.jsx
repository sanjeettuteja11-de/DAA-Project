...
const APPS = [
  ...
  { id:"packSort",   name:"PackSort",     tagline:"E-Commerce Order Sorter",   algo:"Merge Sort + Quick Sort",  paradigm:"Divide & Conquer",    icon:"📦",  accent:"#ff7d3b", industry:"E-Commerce / Retail" },
  ...
  { id:"netSpan",    name:"NetSpan",      tagline:"Fiber Network Planner",     algo:"Kruskal's Algorithm",      paradigm:"Greedy",              icon:"📡",  accent:"#ffcc44", industry:"Telecom / Infrastructure" },
...
/* ════════════════════════════════════════════════
   APP 4: DiffScan — LCS Code Diff Analyzer (ENHANCED)
   ════════════════════════════════════════════════ */
...
const KEYWORDS = { js:["function","async","await","const","let","var","return","if","else","true","false","null","undefined","class","new","import","export","module"], py:["def","class","return","if","elif","else","import","from","True","False","None","raise","self","not","and","or","in"], json:[], css:["linear-gradient","rgba","px","rem","solid","none","auto","flex","grid","block","inline"] };
...
function syntaxColor(line, lang) {
  ...
  let remaining = line;
  // simple tokenizer
  const commentChar = lang==="py"?"#":lang==="css"?"/*":"//";
...
function DiffScan({ accent="#bf7fff" }) {
  ...
  const step = result?.steps?.[p.idx];
  const changesOnly = diff.filter(d=>d.type!=="same");
  const changeGroups = [];
  let cur=[];
  diff.forEach((d,i)=>{
    if(d.type!=="same"){ cur.push({...d,idx:i}); } else {
      if(cur.length){ changeGroups.push(cur); cur=[]; }
    }
  });
  if(cur.length) changeGroups.push(cur);
      ...
      {result&&(
        <div style={{ marginBottom:14 }}>
          <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:T.sub, marginBottom:4 }}>
            <span>File Similarity</span><span style={{ color:sim>=70?"#2ea043":sim>=40?"#ffcc44":"#f85149", fontWeight:700 }}>{sim}%</span>
          </div>
          <div style={{ height:6, background:T.dim, borderRadius:99, overflow:"hidden" }}>
            <div style={{ width:`${sim}%`, height:"100%", borderRadius:99, transition:"width .8s ease",
              background:`linear-gradient(90deg,#f85149,#ffcc44,#2ea043)` }}/>
          </div>
        </div>
      )}
      ...
      {result&&(
        <>
          <div style={{ display:"flex", gap:4, marginBottom:0, borderBottom:`1px solid ${T.border}` }}>
            {[["diff","📄 Diff View"],["changes","⚡ Changes Only"],["dp","🧬 DP Table"],["log","📋 Trace Log"]].map(([id,label])=>(
          ...
          <div style={{ background:"#080810", border:`1px solid ${T.border}`, borderTop:"none", borderRadius:"0 8px 8px 8px", overflow:"hidden" }}>
            ...
            {tab==="changes"&&(
              <div style={{ maxHeight:360, overflowY:"auto", padding:12 }}>
                {changeGroups.length===0&&<div style={{ padding:20, textAlign:"center", color:T.sub, fontSize:13 }}>✅ No changes — files are identical</div>}
                {changeGroups.map((grp,gi)=>(
                  <div key={gi} style={{ marginBottom:16 }}>
                    <div style={{ fontSize:10, color:T.sub, fontFamily:"'DM Mono',monospace", marginBottom:6, padding:"3px 8px", background:T.dim, borderRadius:4, display:"inline-block" }}>@@ Change block {gi+1} @@</div>
                    {grp.map((d,i)=>(
                      <div key={i} style={{ display:"flex", gap:8, padding:"3px 10px", background:d.type==="add"?"#0d2a1a":"#2a0d0d", borderRadius:4, marginBottom:2, fontFamily:"'DM Mono',monospace", fontSize:12 }}>
                        <span style={{ color:d.type==="add"?"#2ea043":"#f85149", fontWeight:800, width:12 }}>{d.type==="add"?"+":"−"}</span>
                        <span style={{ color:d.type==="add"?"#7ee787":"#ff7b72", flex:1 }}>{d.a||d.b}</span>
                      </div>
                    ))}
                  </div>
                ))}
            ...
            {tab==="dp"&&(
              <div style={{ padding:12 }}>
                <div style={{ fontSize:12, color:T.sub, marginBottom:10, fontFamily:"'DM Mono',monospace" }}>
                  DP table fill — dp[i][j] = longest common subsequence of A[0..i] and B[0..j]
                </div>
                <PlayerBar idx={p.idx} total={result.steps.length} playing={p.playing} onPlay={()=>p.setPlaying(x=>!x)} onPrev={p.prev} onNext={p.next} onReset={p.reset} accent={accent}/>
...
/* ════════════════════════════════════════════════
   APP 5: NetSpan — Fiber Network Planner (ENHANCED)
   ════════════════════════════════════════════════ */
const OFFICES = {
  HQ:         { pos:[130,90],  type:"hq",   label:"HQ",          city:"Headquarters",   floors:12 },
  DataCenter: { pos:[310,65],  type:"dc",   label:"DC",          city:"Data Center",    floors:3  },
  North:      { pos:[430,130], type:"branch",label:"North",      city:"North Branch",   floors:4  },
  East:       { pos:[460,250], type:"branch",label:"East",       city:"East Branch",    floors:6  },
  South:      { pos:[300,340], type:"branch",label:"South",      city:"South Branch",   floors:5  },
  West:       { pos:[100,260], type:"branch",label:"West",       city:"West Branch",    floors:4  },
  CentralHub: { pos:[250,195], type:"hub",  label:"Hub",         city:"Central Hub",    floors:8  },
};
const CABLES = [
  ["HQ","DataCenter",12,800], ["HQ","West",8,600],     ["HQ","CentralHub",10,700],
  ["DataCenter","North",6,1000], ["DataCenter","CentralHub",9,900], ["DataCenter","East",15,500],
  ["North","East",7,800],   ["CentralHub","South",11,700], ["CentralHub","East",13,600],
  ["West","South",14,400],  ["East","South",5,1200],
];
// [from, to, cost_lakhs, bandwidth_mbps]

const NODE_ICONS = { hq:"🏛️", dc:"🖥️", hub:"🔄", branch:"🏢" };
const NODE_COLORS = { hq:"#ffcc44", dc:"#4d9fff", hub:"#00c896", branch:"#bf7fff" };

function kruskal() {
  const sorted=[...CABLES].sort((a,b)=>a[2]-b[2]);
  const par={},rnk={};
  Object.keys(OFFICES).forEach(n=>{ par[n]=n; rnk[n]=0; });
  function find(x){ return par[x]===x?x:(par[x]=find(par[x])); }
  function union(x,y){ const px=find(x),py=find(y); if(rnk[px]<rnk[py])par[px]=py; else if(rnk[px]>rnk[py])par[py]=px; else{par[py]=px;rnk[px]++;} }
  const steps=[]; const mst=[]; let tw=0;
  steps.push({ mst:[],cur:null, log:`Sort ${sorted.length} cable routes by cost: ${sorted.map(e=>`${e[0]}-${e[1]}(₹${e[2]}L)`).join(", ")}` });
  for(const cable of sorted) {
    const [u,v,w,bw]=cable;
    steps.push({ mst:[...mst], cur:[u,v,w,bw], log:`Consider ${u} ↔ ${v}: ₹${w}L, ${bw}Mbps` });
    if(find(u)!==find(v)){ union(u,v); mst.push([u,v,w,bw]); tw+=w;
      steps.push({ mst:[...mst], added:[u,v,w,bw], cur:[u,v,w,bw], log:`✅ Lay cable ${u}↔${v}  Cost:₹${w}L  BW:${bw}Mbps  Running total:₹${tw}L` });
    } else {
      steps.push({ mst:[...mst], rej:[u,v,w,bw], cur:[u,v,w,bw], log:`❌ Reject ${u}↔${v} — would create a redundant loop` });
    }
  }
  steps.push({ mst:[...mst], done:true, log:`✅ Optimal network: ₹${tw}L total  |  ${mst.length} links  |  All ${Object.keys(OFFICES).length} offices connected` });
  return { steps, totalCost:tw, mst, totalAll:CABLES.reduce((s,e)=>s+e[2],0) };
}

function NetSpan({ accent="#ffcc44" }) {
  const [result, setResult] = useState(null);
  ...
  const run = ()=>{ const r=kruskal(); setResult(r); p.reset(); setTab("map"); };

  const step = result?.steps?.[p.idx];
  const mst = step?.mst||[];
  const inMST=(u,v)=>mst.some(([a,b])=>(a===u&&b===v)||(a===v&&b===u));
  const isCur=(u,v)=>step?.cur&&((step.cur[0]===u&&step.cur[1]===v)||(step.cur[0]===v&&step.cur[1]===u));
  const isRej=(u,v)=>step?.rej&&((step.rej[0]===u&&step.rej[1]===v)||(step.rej[0]===v&&step.rej[1]===u));
  const mstNodes=new Set(mst.flatMap(([u,v])=>[u,v]));
  const mstCost=mst.reduce((s,e)=>s+e[2],0);
  const totalCost=CABLES.reduce((s,e)=>s+e[2],0);
  const mstBW=mst.reduce((s,e)=>s+(e[3]||0),0);

  return (
    <div>
      {/* Toolbar */}
      <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:12, flexWrap:"wrap" }}>
        <div style={{ fontSize:13, color:T.sub, flex:1 }}>
          {Object.keys(OFFICES).length} offices · {CABLES.length} possible routes · Find minimum-cost fiber layout
        </div>
        <button onClick={run} style={{ ...btnBase, background:accent, color:"#000", fontWeight:700 }}>📡 Plan Network</button>
      </div>

      {/* Tabs */}
      <div style={{ display:"flex", gap:4, marginBottom:0, borderBottom:`1px solid ${T.border}` }}>
        {[["map","🗺️ Network Map"],["breakdown","📊 Cost Breakdown"],["report","📋 Report"]].map(([id,label])=>(
          <button key={id} onClick={()=>setTab(id)} style={{ ...btnBase, padding:"8px 14px", fontSize:12, borderRadius:"6px 6px 0 0",
            border:`1px solid ${tab===id?T.border:"transparent"}`, borderBottom:`1px solid ${tab===id?"#0a0a0f":"transparent"}`,
            marginBottom:-1, background:tab===id?"#0a0a0f":"transparent", color:tab===id?accent:T.sub }}>{label}</button>
        ))}
      </div>

      <div style={{ background:"#0a0a0f", border:`1px solid ${T.border}`, borderTop:"none", borderRadius:"0 8px 8px 8px", overflow:"hidden" }}>

        {/* ── MAP TAB ── */}
        {tab==="map"&&(
          <div>
            {/* Legend */}
            <div style={{ display:"flex", gap:12, padding:"8px 14px", borderBottom:`1px solid ${T.border}`, flexWrap:"wrap" }}>
              {Object.entries(NODE_COLORS).map(([type,color])=>(
                <div key={type} style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:T.sub }}>
                  <div style={{ width:10,height:10,borderRadius:2,background:color }}/>
                  <span style={{ textTransform:"capitalize" }}>{NODE_ICONS[type]} {type}</span>
                </div>
              ))}
              <div style={{ marginLeft:"auto", display:"flex", gap:14 }}>
                <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:accent }}>
                  <div style={{ width:20, height:3, background:accent, borderRadius:2 }}/> MST Cable
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:"#ff4d6d" }}>
                  <div style={{ width:20, height:2, background:"#ff4d6d", borderRadius:2, opacity:.5 }}/> Rejected
                </div>
              </div>
            </div>
            ...
            <div style={{ padding:10, overflowX:"auto" }}>
              <svg viewBox="0 0 580 420" style={{ width:"100%", minWidth:380 }}>
                <defs>
                  ...
                  {CABLES.map(([u,v],i)=>{
                    const [x1,y1]=OFFICES[u].pos, [x2,y2]=OFFICES[v].pos;
                    return <linearGradient key={i} id={`lg${i}`} x1={x1} y1={y1} x2={x2} y2={y2} gradientUnits="userSpaceOnUse">
                      <stop offset="0%" stopColor={NODE_COLORS[OFFICES[u].type]}/>
                      <stop offset="100%" stopColor={NODE_COLORS[OFFICES[v].type]}/>
                    </linearGradient>;
                  })}
                </defs>

                {/* Background grid */}
                <rect x={0} y={0} width={580} height={420} fill="url(#grid)" opacity={.3}/>

                {/* Edges */}
                {CABLES.map(([u,v,w,bw],i)=>{
                  const [x1,y1]=OFFICES[u].pos, [x2,y2]=OFFICES[v].pos;
                  const mx=(x1+x2)/2, my=(y1+y2)/2;
                  const onMST=inMST(u,v), cur=isCur(u,v), rej=isRej(u,v);
                  const hovering=hovEdge===i;
                  return (
                    <g key={i} onMouseEnter={()=>setHovEdge(i)} onMouseLeave={()=>setHovEdge(null)} style={{ cursor:"pointer" }}>
                      {/* Glow layer for MST */}
                      {onMST&&<line x1={x1} y1={y1} x2={x2} y2={y2} stroke={accent} strokeWidth={8} opacity={.15} filter="url(#glow2)"/>}
                      {/* Main edge */}
                      <line x1={x1} y1={y1} x2={x2} y2={y2}
                        stroke={onMST?accent:cur?"#ffffff":rej?"#ff4d4d":"#1e1e2e"}
                        strokeWidth={onMST?2.5:cur?2:1}
                        strokeDasharray={onMST?"none":rej?"6,4":"4,4"}
                        opacity={rej?.4:1}
                        filter={onMST?"url(#glow)":"none"}/>
                      {/* Cost bubble */}
                      {(onMST||cur||hovering)&&(
                        <g>
                          <rect x={mx-20} y={my-11} width={40} height={20} rx={4}
                            fill={onMST?"#1a1500":cur?"#111":T.card} stroke={onMST?accent:cur?"#888":T.border} strokeWidth={1}/>
                          <text x={mx} y={my+4} textAnchor="middle" fontSize={9} fill={onMST?accent:cur?"#fff":"#6b6b80"} fontFamily="'DM Mono',monospace" fontWeight="600">₹{w}L</text>
                        </g>
                      )}
                      {/* BW label on MST hover */}
                      {onMST&&hovering&&(
                        <g>
                          <rect x={mx-24} y={my+10} width={48} height={16} rx={3} fill="#0a1a0a" stroke={accent} strokeWidth={1} opacity={.9}/>
                          <text x={mx} y={my+22} textAnchor="middle" fontSize={8} fill="#00c896" fontFamily="'DM Mono',monospace">{bw}Mbps</text>
                        </g>
                      )}
                    </g>
                  );
                ...
                {/* Nodes */}
                {Object.entries(OFFICES).map(([name,{pos:[cx,cy],type,label}])=>{
                  const nc=NODE_COLORS[type];
                  const inNet=mstNodes.has(name)||mst.length===0;
                  const hov=hovNode===name;
                  const isHQ=type==="hq", isDC=type==="dc";
                  const r=isHQ?26:isDC?22:20;
                  return (
                    <g key={name} onMouseEnter={()=>setHovNode(name)} onMouseLeave={()=>setHovNode(null)} style={{ cursor:"pointer" }}>
                      {/* Glow */}
                      {inNet&&mst.length>0&&<circle cx={cx} cy={cy} r={r+8} fill={nc} opacity={.08} filter="url(#glow2)"/>}
                      {/* Node ring */}
                      <circle cx={cx} cy={cy} r={r+3} fill="none" stroke={inNet&&mst.length>0?nc:"#1e1e2e"} strokeWidth={1} opacity={.4}/>
                      {/* Node body */}
                      <circle cx={cx} cy={cy} r={r}
                        fill={inNet&&mst.length>0?`${nc}20`:"#0d0d0d"}
                        stroke={inNet&&mst.length>0?nc:"#2a2a3a"}
                        strokeWidth={inNet&&mst.length>0?2:1}
                        filter={hov?"url(#glow)":"none"}/>
                      {/* Icon */}
                      <text x={cx} y={cy-4} textAnchor="middle" fontSize={14}>{NODE_ICONS[type]}</text>
                      {/* Label */}
                      <text x={cx} y={cy+12} textAnchor="middle" fontSize={9} fill={inNet&&mst.length>0?nc:"#4a4a6a"} fontFamily="'DM Mono',monospace" fontWeight="600">{label}</text>
                      {/* Tooltip */}
                      {hov&&(
                        <g>
                          <rect x={cx-38} y={cy-r-34} width={76} height={28} rx={5} fill={T.card} stroke={nc} strokeWidth={1.5}/>
                          <text x={cx} y={cy-r-22} textAnchor="middle" fontSize={9} fill={nc} fontFamily="'DM Mono',monospace" fontWeight="700">{name}</text>
                          <text x={cx} y={cy-r-11} textAnchor="middle" fontSize={8} fill={T.sub} fontFamily="'DM Mono',monospace">{OFFICES[name].city}</text>
                        </g>
                      )}
                    </g>
                  );
            ...
            {result&&step&&(
              <div style={{ padding:"10px 14px", borderTop:`1px solid ${T.border}` }}>
                <div style={{ padding:"7px 12px", background:step.done?`${accent}12`:step.added?`#00c89612`:step.rej?"#ff4d6d12":"#1a1a2a",
                  border:`1px solid ${step.done?accent:step.added?"#00c896":step.rej?"#ff4d6d":T.border}`,
                  borderRadius:6, fontSize:12, color:step.done?accent:step.added?"#00c896":step.rej?"#ff4d6d":"#8b949e",
                  fontFamily:"'DM Mono',monospace", marginBottom:10 }}>{step.log}</div>
                <PlayerBar idx={p.idx} total={result.steps.length} playing={p.playing} onPlay={()=>p.setPlaying(x=>!x)} onPrev={p.prev} onNext={p.next} onReset={p.reset} accent={accent}/>
              </div>
            )}
          </div>
        ...
        {tab==="breakdown"&&(
          <div style={{ padding:16 }}>
            {!result&&<div style={{ textAlign:"center", padding:30, color:T.sub, fontSize:13 }}>Run the algorithm first to see cost breakdown</div>}
            {result&&(
              <>
                {/* Summary cards */}
                <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, marginBottom:16 }}>
                  {[
                    ["MST Cost",`₹${mstCost}L`,accent,"Minimum cable spend"],
                    ["Total if all",`₹${totalCost}L`,"#ff4d6d","Cost without optimization"],
                    ["Savings",`₹${totalCost-mstCost}L (${Math.round((1-mstCost/totalCost)*100)}%)`,`#00c896`,"Money saved by algorithm"],
                  ].map(([l,v,c,sub])=>(
                    <div key={l} style={{ padding:"14px 12px", background:`${c}10`, border:`1px solid ${c}30`, borderRadius:10 }}>
                      <div style={{ fontSize:10, color:T.sub, marginBottom:4 }}>{l}</div>
                      <div style={{ fontSize:17, fontWeight:800, color:c, marginBottom:2 }}>{v}</div>
                      <div style={{ fontSize:10, color:"#4a4a6a" }}>{sub}</div>
                    </div>
                  ))}
                </div>

                {/* Sorted cable table */}
                <div style={{ fontSize:11, color:T.sub, marginBottom:8, fontFamily:"'DM Mono',monospace" }}>All cable routes sorted by cost (Kruskal processes in this order):</div>
                <div style={{ background:T.card, borderRadius:10, overflow:"hidden", border:`1px solid ${T.border}` }}>
                  <div style={{ display:"grid", gridTemplateColumns:"32px 1fr 1fr 72px 72px 60px", gap:0, padding:"8px 12px",
                    borderBottom:`1px solid ${T.border}`, fontSize:10, color:T.sub, fontFamily:"'DM Mono',monospace", fontWeight:700 }}>
                    <span>#</span><span>From</span><span>To</span><span style={{ textAlign:"right" }}>Cost</span><span style={{ textAlign:"right" }}>BW</span><span style={{ textAlign:"right" }}>Status</span>
                  </div>
                  {[...CABLES].sort((a,b)=>a[2]-b[2]).map(([u,v,w,bw],i)=>{
                    const added=result.mst.some(([a,b])=>(a===u&&b===v)||(a===v&&b===u));
                    return (
                      <div key={i} style={{ display:"grid", gridTemplateColumns:"32px 1fr 1fr 72px 72px 60px", gap:0, padding:"9px 12px",
                        borderBottom:`1px solid ${T.border}`, fontSize:12, background:added?`${accent}08`:"transparent",
                        borderLeft:`3px solid ${added?accent:"transparent"}` }}>
                        <span style={{ color:T.sub, fontSize:10, fontFamily:"'DM Mono',monospace" }}>{i+1}</span>
                        <span style={{ color:T.text, fontWeight:added?600:400, display:"flex", alignItems:"center", gap:4 }}>
                          <span style={{ fontSize:10 }}>{NODE_ICONS[OFFICES[u].type]}</span>{u}
                        </span>
                        <span style={{ color:T.text, fontWeight:added?600:400, display:"flex", alignItems:"center", gap:4 }}>
                          <span style={{ fontSize:10 }}>{NODE_ICONS[OFFICES[v].type]}</span>{v}
                        </span>
                        <span style={{ color:added?accent:T.sub, textAlign:"right", fontFamily:"'DM Mono',monospace", fontSize:12, fontWeight:added?700:400 }}>₹{w}L</span>
                        <span style={{ color:"#00c89688", textAlign:"right", fontFamily:"'DM Mono',monospace", fontSize:11 }}>{bw}M</span>
                        <span style={{ textAlign:"right" }}>
                          {added
                            ? <span style={{ ...pill(accent), fontSize:10 }}>✅ Laid</span>
                            : <span style={{ ...pill("#ff4d6d"), fontSize:10 }}>❌ Loop</span>}
                        </span>
                      </div>
                    );
                  })}
                ...
                {/* Bandwidth bar */}
                <div style={{ marginTop:14, padding:14, background:T.dim, borderRadius:10 }}>
                  <div style={{ fontSize:12, color:T.sub, marginBottom:10 }}>Network Bandwidth Distribution (MST links)</div>
                  {result.mst.sort((a,b)=>b[3]-a[3]).map(([u,v,w,bw],i)=>(
                    <div key={i} style={{ marginBottom:8 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:T.sub, marginBottom:3 }}>
                        <span style={{ fontFamily:"'DM Mono',monospace" }}>{u} ↔ {v}</span>
                        <span style={{ color:accent }}>{bw} Mbps</span>
                      </div>
                      <div style={{ height:5, background:"#1a1a2a", borderRadius:99, overflow:"hidden" }}>
                        <div style={{ width:`${(bw/1200)*100}%`, height:"100%", background:`linear-gradient(90deg,${accent},#00c896)`, borderRadius:99 }}/>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {tab==="report"&&(
          <div style={{ padding:16 }}>
            {!result&&<div style={{ textAlign:"center", padding:30, color:T.sub, fontSize:13 }}>Run the algorithm first to generate report</div>}
            {result&&(
              <>
                <div style={{ display:"flex", alignItems:"center", gap:10, padding:"12px 16px", background:`${accent}10`, border:`1px solid ${accent}30`, borderRadius:10, marginBottom:16 }}>
                  <span style={{ fontSize:28 }}>📡</span>
                  <div>
                    <div style={{ fontSize:15, fontWeight:700, color:accent }}>Minimum Spanning Network — Final Report</div>
                    <div style={{ fontSize:12, color:T.sub }}>Kruskal's Algorithm · Union-Find · O(E log E)</div>
                  </div>
                </div>

                {[
                  { label:"Network Topology", rows:[
                    ["Offices connected",Object.keys(OFFICES).length],
                    ["Fiber links laid",result.mst.length],
                    ["Links rejected",CABLES.length-result.mst.length],
                    ["Algorithm steps",result.steps.length],
                  ]},
                  { label:"Cost Analysis", rows:[
                    ["MST total cost",`₹${mstCost} Lakhs`],
                    ["Naïve cost (all cables)",`₹${totalCost} Lakhs`],
                    ["Savings",`₹${totalCost-mstCost} Lakhs`],
                    ["Optimization rate",`${Math.round((1-mstCost/totalCost)*100)}%`],
                  ]},
                  { label:"Performance", rows:[
                    ["Total bandwidth (MST)",`${mstBW} Mbps`],
                    ["Avg cost per link",`₹${Math.round(mstCost/result.mst.length*10)/10}L`],
                    ["Highest BW link",result.mst.sort((a,b)=>b[3]-a[3])[0]?`${result.mst[0][0]}↔${result.mst[0][1]} (${result.mst[0][3]}Mbps)`:"—"],
                    ["Cheapest MST link",result.mst.sort((a,b)=>a[2]-b[2])[0]?`${result.mst[0][0]}↔${result.mst[0][1]} (₹${result.mst[0][2]}L)`:"—"],
                  ]},
                ].map(section=>(
                  <div key={section.label} style={{ marginBottom:14, background:T.card, border:`1px solid ${T.border}`, borderRadius:10, overflow:"hidden" }}>
                    <div style={{ padding:"8px 14px", background:T.dim, borderBottom:`1px solid ${T.border}`, fontSize:11, color:accent, fontWeight:700, textTransform:"uppercase", letterSpacing:1 }}>{section.label}</div>
                    {section.rows.map(([k,v])=>(
                      <div key={k} style={{ display:"flex", justifyContent:"space-between", padding:"9px 14px", borderBottom:`1px solid ${T.border}`, fontSize:13 }}>
                        <span style={{ color:T.sub }}>{k}</span>
                        <span style={{ color:T.text, fontFamily:"'DM Mono',monospace", fontWeight:600 }}>{v}</span>
                      </div>
                    ))}
                  </div>
                ))}
...
/* ════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════ */
const APP_COMPONENTS = { naviRoute:NaviRoute, packSort:PackSort, zipIt:ZipIt, diffScan:DiffScan, netSpan:NetSpan, shiftBoard:ShiftBoard };
...
function RealWorldInfo({ appId, accent }) {
  const info = {
    ...
    netSpan:   { who:"BSNL, Jio, Tata Comm, Reliance Jio, AWS", why:"Laying redundant fiber is enormously expensive. Kruskal's MST connects every office with minimum total cable, zero loops, using Union-Find for cycle detection.", impact:"Saves ₹crores per project — used in every large network rollout." },
...
function AlgoInfo({ appId, accent }) {
  const info = {
    ...
    netSpan:    { time:"O(E log E)", space:"O(V+E)", paradigm:"Greedy", note:"Union-Find detects cycles in near-O(1) amortized" },
    ...
    <div style={{ width:"100%", maxWidth:"100%", margin:0, padding:"48px 20px" }}>
      ...
      <div style={{ marginBottom:52 }}>
        ...
        <p style={{ fontSize:15, color:"#6868a0", maxWidth:520, lineHeight:1.8, margin:0 }}>
          Six production-grade tools, each powered by a core DAA algorithm. See how the theory you learned actually runs the world around you.
        </p>
      </div>
      ...
      <div style={{ ...card(), padding:28, marginBottom:28 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:12 }}>
          {[
            ...
            { name:"Greedy", desc:"Local optimal at each step", app:"NaviRoute, ZipIt, NetSpan", color:"#00c896" },
      ...
      {!activeApp&&(
        <nav style={{ background:T.card, borderBottom:`1px solid ${T.border}`, padding:"0 24px", height:52, display:"flex", alignItems:"center", gap:10, position:"sticky", top:0, zIndex:50 }}>
          ...
          <div style={{ marginLeft:"auto", display:"flex", gap:6 }}>
            <Tag label="7 Algorithms" color="#4d9fff"/>
